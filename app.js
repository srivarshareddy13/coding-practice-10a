...
const convertToDistrict = dbObject => {
  return {
...
app.get('/states/', authentication, async (request, response) => {
  ...
  response.send(result.map((each) => (convertToState(each))))
...
app.get("/states/:stateId/", authentication, async (request,response) => {
  ...
  `;
   ...
   response.send(convertToState(result))
});
...
app.get("/districts/:districtId/", authentication, async (request,response) => {
  ...
  `;
   const result = await db.get(getQuery) 
   response.send(convertToDistrict(result))
}); 
